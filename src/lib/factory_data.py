...
class SQLPostgres(DataSource):
    ...
    def __init__(self, **parametros) -> None:
        ...
        self.query_read = _file_path.joinpath(
            "src/data/querys/get_table.sql"
        ).as_posix()
        self.query_write = _file_path.joinpath(
            "src/data/querys/insert_data.sql"
        ).as_posix()
        self.query_create = _file_path.joinpath(
            "src/data/querys/new_table.sql"
        ).as_posix()
    ...
    def read(self):
        ...
        query = self.data_source.prepare_query_replace_value(
            sql_file=self.query_read, data_replace=fix_dict_query
        )

        return self.data_source.get_table(
            connection_parameters=self.parametro.connection_data_source, query=query
        )
    ...
    def write(self, data: pd.DataFrame):
        ...
        query = self.data_source.prepare_query_replace_value(
            sql_file=self.query_write, data_replace=fix_dict_query
        )
    ...
    def create(self):
        ...
        query = self.data_source.prepare_query_replace_value(
            sql_file=self.query_create, data_replace=fix_data_dict
        )

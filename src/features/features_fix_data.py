...
class ColumnsNameHandler:
    ...
    def check_values_range(
        self, dict_types: dict, min_val: float, max_val: float, dict_out: dict, key: str
    ) -> dict:
        """
    ...
    def variables_resize(self, dataframe):
        ...
        integer_signed = {
            "int8": {0: np.iinfo(np.int8).min, 1: np.iinfo(np.int8).max},
            "int16": {0: np.iinfo(np.int16).min, 1: np.iinfo(np.int16).max},
            "int32": {0: np.iinfo(np.int32).min, 1: np.iinfo(np.int32).max},
            "int64": {0: np.iinfo(np.int64).min, 1: np.iinfo(np.int64).max},
        }

        integer_unsigned = {
            "uint8": {0: np.iinfo(np.uint8).min, 1: np.iinfo(np.uint8).max},
            "uint16": {0: np.iinfo(np.uint16).min, 1: np.iinfo(np.uint16).max},
            "uint32": {0: np.iinfo(np.uint32).min, 1: np.iinfo(np.uint32).max},
            "uint64": {0: np.iinfo(np.uint64).min, 1: np.iinfo(np.uint64).max},
        }

        floating = {
            "float16": {0: np.finfo(np.float16).min, 1: np.finfo(np.float16).max},
            "float32": {0: np.finfo(np.float32).min, 1: np.finfo(np.float32).max},
            "float64": {0: np.finfo(np.float64).min, 1: np.finfo(np.float64).max},
        }
        ...
        for cont, key in enumerate(features):
            ...
            # Preguntar si el valor minimo tiene valores negativos
                ...
                new_dtypes = self.check_values_range(
                    integer_unsigned, min_ft, max_ft, new_dtypes, key
                )
            ...
            # Preguntar si el valor minimo tiene valores negativos
                ...
                new_dtypes = self.check_values_range(
                    integer_signed, min_ft, max_ft, new_dtypes, key
                )
...
class PrepareData(ColumnsNameHandler):
    ...
    def transforf_dataframe_dart(self, time_col: str, data_col: str):
        ...
        data_for_model = TimeSeries.from_dataframe(data, time_col, [data_col])
    ...
    def metrics_column(self, dataframe: pd.DataFrame):
        ...
        return {
            "Rango": round(dataframe.max() - dataframe.min(), 3),
            "Varianza": round(dataframe.var(), 3),
            "Desviacion_estandar": round(dataframe.std(), 3),
            "Coeficiente_varianza": round(
                (dataframe.std() / dataframe.mean()) * 100, 3
            ),  # Resultaod en porcentaje)
    ...
    def display_forecast(
        self, pred_series, ts_transformed, forecast_type: str, start_date: str = None
    ):
...
if __name__ == "__main__":
    ...
    strategy_ = {int: np.mean, float: np.mean, object: stats.mode}
